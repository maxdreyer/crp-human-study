...
from utils.milan.deps.pretorched import data, models, optim, utils
from utils.milan.deps.pretorched.data import samplers, transforms
...
def get_optimizer(model, optimizer_name='SGD', lr=0.001, **kwargs):
    ...
    optim_kwargs = {
        **cfg.optimizer_defaults.get(optimizer_name, {}),
        **func_kwargs
    }
...
def get_scheduler(optimizer, scheduler_name='CosineAnnealingLR', **kwargs):
    ...
    sched_kwargs = {
        **cfg.scheduler_defaults.get(scheduler_name, {}),
        **func_kwargs
    }
    ...
    if pretrained is not None:
        ...
        if nc != num_classes:
            ...
            if init_name is not None:
                print(
                    f'Re-initializing last_linear of {model_name} with {init_name}.'
                )
...
def get_video_dataloader(
    ...
    **kwargs,
    ...
    loader_sampler = (DistributedSampler(dataset) if
                      (distributed and split == 'train') else None)
    return DataLoader(
        dataset,
...
def get_hybrid_dataset(
    ...
    imagenet_root = cfg.get_root_dirs('ImageNet',
                                      dataset_type=dataset_type,
                                      resolution=resolution,
                                      data_root=root)
    places365_root = cfg.get_root_dirs('Places365',
                                       dataset_type=dataset_type,
                                       resolution=resolution,
                                       data_root=root)
    ...
    return torch.utils.data.ConcatDataset(
        (imagenet_dataset, placess365_dataset))
...
def get_dataloader(
    ...
    **kwargs,
    ...
    sampler = (DistributedSampler(dataset) if
               (distributed and split == 'train') else None)
    return DataLoader(
        dataset,
        batch_size=batch_size,
    ...
    if os.path.isfile(checkpoint_path):
        ...
        try:
            ...
            try:
                ...
                print(
                    f'setting scheduler learning rate to: {scheduler.get_lr()}'
                )
            except Exception:
                pass
