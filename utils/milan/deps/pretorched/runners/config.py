...
import utils.milan.deps.pretorched.models as models
...
def get_root_dirs(name,
    ...
    root_dirs = {
        'ImageNet': {
            'ImageHDF5':
                defaultdict(lambda: os.path.join(data_root, 'ImageNet'), {}),
            'ImageFolder':
                defaultdict(lambda: os.path.join(data_root, 'ImageNet'), {}),
        },
        'Places365': {
            'ImageHDF5':
                defaultdict(lambda: os.path.join(data_root, 'Places365'), {}),
            'ImageFolder':
                defaultdict(lambda: os.path.join(data_root, 'Places365'), {}),
        },
        'Hybrid1365': {
        ...
        'M3': {
            'VideoRecordZipDataset':
                defaultdict(lambda: os.path.join(data_root, 'Moments/videos'))
        }
    }
...
def get_metadata(name,
    ...
    root_dirs = {
        'DFDC': {
            ...
            'DeepfakeFaceFrame':
                defaultdict(
                    lambda: os.path.join(
                        data_root, 'DeepfakeDetection/facenet_frames'), {}),
        ...
        'Kinetics': {
            'VideoRecordDataset':
                defaultdict(lambda: os.path.join(data_root, 'Kinetics/videos')
                           ),
            'VideoRecordZipDataset':
                defaultdict(lambda: os.path.join(data_root, 'Kinetics/videos')
                           ),
    ...
    metafiles = {
        'Moments': {
            'RecordSet':
                defaultdict(lambda: os.path.join(data_root, 'Moments', fname),
                            {}),
        },
        'M3': {
            'RecordSet':
                defaultdict(lambda: os.path.join(data_root, 'M3', fname), {}),
        },
        'Kinetics': {
            'RecordSet':
                defaultdict(lambda: os.path.join(data_root, 'Kinetics', fname),
                            {}),
        },
        'MultiMoments': {
            'RecordSet':
...
def name_from_args(args):
    name = '_'.join([
        item for item in [
            args.arch,
            args.dataset.lower(),
            f'seg_count-{args.segment_count}' if args.dataset in
            VIDEO_DATASETS else None,
