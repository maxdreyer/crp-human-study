...
class GBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if self.learnable_sc:
            self.conv_sc = self.conv_func(in_channels,
                                          out_channels,
                                          kernel_size=1,
                                          padding=0)
        # Batchnorm layers
...
class Generator(nn.Module):
    ...
    def __init__(self,
        ...
        if self.G_param == 'SN':
            ...
            self.conv_func = functools.partial(nn.Conv2d,
                                               kernel_size=3,
                                               padding=1)
        ...
        bn_linear = (functools.partial(self.linear_func, bias=False)
                     if self.G_shared else self.embedding_func)
        ...
        self.shared = (self.embedding_func(n_classes, self.shared_dim)
                       if G_shared else nn.Identity())
        # First linear layer
...
class Discriminator(nn.Module):
    ...
    def __init__(self,
        ...
        self.linear = self.linear_func(self.arch['out_channels'][-1],
                                       output_dim)
        # Embedding for projection discrimination
        self.embed = self.embedding_func(self.n_classes,
...
model_urls = {
    'places365': {
        128: {
            96: {
                'D':
                    os.path.join(root_url,
                                 'biggan128_D_places365-8afb2a4d.pth'),
                'G':
                    os.path.join(root_url,
                                 'biggan128_G_places365-43cd58c0.pth'),
                'G_ema':
                    os.path.join(root_url,
                                 'biggan128_G_ema_places365-78c78abe.pth'),
                'state_dict':
                    os.path.join(
                        root_url,
                        'biggan128_state_dict_places365-3d39f6bb.pth')
            }
        },
        256: {
            96: {
                'D':
                    os.path.join(root_url,
                                 'biggan256_D_ch96_places365-44bf2902.pth'),
                'G':
                    os.path.join(root_url,
                                 'biggan256_G_ch96_places365-5adac787.pth'),
                'G_ema':
                    os.path.join(
                        root_url,
                        'biggan256_G_ema_ch96_places365-ac277771.pth'),
                'state_dict':
                    os.path.join(
                        root_url,
                        'biggan256_state_dict_ch96_places365-b4f6daf6.pth'),
            },
            128: {
                'D':
                    os.path.join(root_url,
                                 'biggan256_D_ch128_places365-a6f7d3b6.pth'),
                'G':
                    os.path.join(root_url,
                                 'biggan256_G_ch128_places365-47f6e48c.pth'),
                'G_ema':
                    os.path.join(
                        root_url,
                        'biggan256_G_ema_ch128_places365-6fb66feb.pth'),
                'state_dict':
                    os.path.join(
                        root_url,
                        'biggan256_state_dict_ch128_places365-7594847d.pth'),
            }
        },
    },
    'imagenet': {
        128: {
            96: {
                'D':
                    os.path.join(root_url,
                                 'biggan128_D_imagenet-9fd72e50.pth'),
                'G':
                    os.path.join(root_url,
                                 'biggan128_G_imagenet-94e0b761.pth'),
                'G_ema':
                    os.path.join(root_url,
                                 'biggan128_G_ema_imagenet-c9706dfb.pth'),
                'state_dict':
                    os.path.join(root_url,
                                 'biggan128_state_dict_imagenet-4aad5089.pth'),
            }
        },
    }
}
