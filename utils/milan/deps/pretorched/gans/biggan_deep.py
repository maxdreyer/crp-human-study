...
class GBlock(nn.Module):
    ...
    def __init__(self,
        ...
        # Conv layers
        self.conv1 = self.conv_func(self.in_channels,
        ...
        self.conv4 = self.conv_func(self.hidden_channels,
                                    self.out_channels,
                                    kernel_size=1,
                                    padding=0)
        # Batchnorm layers
    ...
    def __init__(self,
        ...
        self.bn_func = functools.partial(
            ...
            input_size=(self.shared_dim +
                        self.dim_z if self.G_shared else self.n_classes),
        ...
        for index in range(len(self.arch['out_channels'])):
            self.blocks += [[
                GBlock(
                    ...
                    if g_index == 0 else self.arch['out_channels'][index],
                    conv_func=self.conv_func,
                    bn_func=self.bn_func,
                    activation=self.activation,
                    upsample=(functools.partial(F.interpolate, scale_factor=2)
                              if self.arch['upsample'][index] and
                              g_index == (self.G_depth - 1) else None))
            ...
            self.blocks += [[
                DBlock(in_channels=self.arch['in_channels'][index]
                       ...
                       downsample=(nn.AvgPool2d(2)
                                   if self.arch['downsample'][index] and
                                   d_index == 0 else None))
    ...
    'places365-challenge': {
        256: {
            'D':
                os.path.join(
                    root_url,
                    'biggan_deep256_D_places365-challenge-c4fb8bfe.pth'),
            'G':
                os.path.join(
                    root_url,
                    'biggan_deep256_G_places365-challenge-1d2bae3c.pth'),
            'G_ema':
                os.path.join(
                    root_url,
                    'biggan_deep256_G_ema_places365-challenge-c3a49c8a.pth'),
            'state_dict':
                os.path.join(
                    root_url,
                    'biggan_deep256_state_dict_places365-challenge-dadfc659.pth'
                )
        }
    }
}
...
def BigGANDeep(resolution=256,
               pretrained='places365-challenge',
               load_ema=True):
