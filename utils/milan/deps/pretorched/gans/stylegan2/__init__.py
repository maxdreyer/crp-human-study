...
from utils.milan.deps.pretorched.gans.stylegan import RESOLUTIONS
...
model_urls = {
    'bedroom': {
        256: {
            'G': os.path.join(root_url, 'bedroom_256x256_G-11072c2b.pth')
        }
    },
    'car': {
        512: {
            'G': os.path.join(root_url, 'car_512x512_G-0ce43c2d.pth')
        }
    },
    'cat': {
        256: {
            'G': os.path.join(root_url, 'cat_256x256_G-f0b1bf2c.pth')
        }
    },
    'church': {
        256: {
            'G': os.path.join(root_url, 'church_256x256_G-905a491c.pth')
        }
    },
    'faces': {
        1024: {
            'G': os.path.join(root_url, 'faces_1024x1024_G-c10dbc80.pth')
        }
    },
    'horse': {
        256: {
            'G': os.path.join(root_url, 'horse_256x256_G-524e68e4.pth')
        }
    },
    'kitchen': {
        256: {
            'G': os.path.join(root_url, 'kitchen_256x256_G-d7083483.pth')
        }
    },
    'places': {
        256: {
            'G': os.path.join(root_url, 'places_256x256_G-47082c7d.pth')
        }
    },
}
...
def stylegan2(pretrained='bedroom',
              truncation=1.0,
              resolution=None,
              **kwargs):  # mconv='seq'):
    ''' loads nn sequential version of stylegan2.'''
    if pretrained is not None:
        resolution = sizes.get(pretrained,
                               256) if resolution is None else resolution
        url = model_urls[pretrained][resolution]['G']
        state_dict = torch.hub.load_state_dict_from_url(url)
        g = SeqStyleGAN2(resolution,
                         style_dim=512,
                         n_mlp=8,
                         truncation=truncation,
                         **kwargs)
...
def z_sample_for_model(model, size=100, seed=1):
    ...
    if hasattr(model, 'input_shape'):
        sample = standard_z_sample(size, model.input_shape[1],
                                   seed=seed).view((size,) +
                                                   model.input_shape[1:])
        return sample
    ...
    if isinstance(first_layer, (torch.nn.Conv2d, torch.nn.ConvTranspose2d)):
        sample = standard_z_sample(size, first_layer.in_channels,
                                   seed=seed)[:, :, None, None]
    else:
...
def standard_z_sample(size, depth, seed=1, device=None):
    ...
    result = torch.from_numpy(
        rng.standard_normal(size * depth).reshape(size, depth)).float()
